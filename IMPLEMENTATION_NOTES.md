...
CrDetailComponent
- canApprove: boolean (getter) — true only when the loaded CR's status is 'PENDING_APPROVAL' 
  and the current session user holds an approve-scope policy (cr_a_u / cr_a_w / cr_a_o), 
  via canApprovePolicy(); drives whether the approve action is shown/enabled in the template.
- rejectControl: FormControl<string> — bound to the reject-reason input; exposes .invalid 
  to gate the reject button and show the error message.
...
| Invariant | How / where |
|---|---|
| Reject action cannot be submitted with an empty or whitespace-only reason | notBlank + Validators.required on 
rejectControl in cr-detail.component.ts; submit button uses [disabled]="rejectControl.invalid" |
